...
class ComplianceService {
    ...
    poolRepo;
    routeRepo;
    constructor(complianceRepo, poolRepo, routeRepo) {
    ...
    async getComplianceBalance(shipId, year) {
        ...
        const avgIntensity = routes.reduce((sum, r) => sum + r.ghg_intensity, 0) / routes.length;
    ...
    async bankPositiveCb(shipId, year, amount) {
        const currentCb = await this.getComplianceBalance(shipId, year);
        if (currentCb <= 0)
            throw new Error("CB must be positive to bank surplus");
        if (amount <= 0)
            throw new Error("Amount must be greater than zero");
        if (amount > currentCb)
            throw new Error(`Cannot bank more than current CB (${currentCb.toFixed(2)})`);
    ...
    async applyBankedSurplus(shipId, year, amount) {
        const bankedAmount = await this.complianceRepo.getBankedAmount(shipId, year);
        if (bankedAmount <= 0)
            throw new Error("No banked surplus available");
        if (amount <= 0)
            throw new Error("Amount must be greater than zero");
        if (amount > bankedAmount)
            throw new Error(`Insufficient banked surplus. Available: ${bankedAmount.toFixed(2)}`);
        const currentCb = await this.getComplianceBalance(shipId, year);
        const newCb = currentCb + amount;
        await this.complianceRepo.saveComplianceBalance(shipId, year, newCb);
    ...
    async createPool(year, members) {
        ...
        if (availableSurplus > 0) {
            ...
            if (surplusShip)
                surplusShip.cb_after = availableSurplus;
        }
